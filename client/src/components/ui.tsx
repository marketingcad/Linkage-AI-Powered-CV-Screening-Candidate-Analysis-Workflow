      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-4 border-b border-slate-50 px-4 py-4 last:border-0">
          ...
          <div className="flex-1 space-y-2">
            <Skeleton className="skeleton-shimmer h-3.5 w-1/3" />
            <Skeleton className="skeleton-shimmer h-3 w-1/4" />
          </div>
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="var(--color-slate-200)" strokeWidth={5} />
        <circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
